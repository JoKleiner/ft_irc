...
bool Server::check_ChaOpCo_input(const std::vector<std::string> &token)
{
	...
	if (chan_ele == _channels.end())
		return (sendRplErr(_clients[_iter], SERVERNAME, "403", (token[0] == "INVITE" ? token[2] : token[1]) + " :No such channel"), false);
	...
	if (client == clie_list.end())
		return (sendRplErr(_clients[_iter], SERVERNAME, "442", channelName + " :You're not on that channel"), false);
	if (token[0] != "TOPIC" && !client->second.ch_operator)
		return (sendRplErr(_clients[_iter], SERVERNAME, "482", channelName + " :You're not channel operator"), false);

	client = clie_list.find(token[1]);

	if (token[0] == "INVITE" && client != clie_list.end())
		return (sendRplErr(_clients[_iter], SERVERNAME, "443", token[1] + " " + channelName + " :is already on channel"), false);
	return true;
}
...
bool Server::check_kick(std::string &channel, std::string &nick_splits)
{
	...
	if (chan_ele == _channels.end())
		return (sendRplErr(_clients[_iter], SERVERNAME, "403", channel + " :No such channel"), false);
	auto clie_list = chan_ele->second.get_cha_cl_list();
	auto client = clie_list.find(_clients[_iter].get_nick());
	if (client == clie_list.end())
		return (sendRplErr(_clients[_iter], SERVERNAME, "442", channel + " :You're not on that channel"), false);
	if (!client->second.ch_operator)
		return (sendRplErr(_clients[_iter], SERVERNAME, "482", channel + " :You're not channel operator"), false);

	client = clie_list.find(nick_splits);
	if (client == clie_list.end())
		return (sendRplErr(_clients[_iter], SERVERNAME, "441", nick_splits + " " + channel +" :They aren't on that channel"), false);
	if (client->second.ch_operator)
		return (sendRplErr(_clients[_iter], SERVERNAME, "482", channel + " :You're not allowed to kick operator"), false);
	return true;
}
...
{
	if (token.size() < 3)
		return (sendRplErr(_clients[_iter], SERVERNAME, "461", "KICK :Not enough parameters"), void());
...
{
	if (token.size() < 3)
		return (sendRplErr(_clients[_iter], SERVERNAME, "461", "INVITE :Not enough parameters"), void());
...
{
	if (token.size() < 2)
		return (sendRplErr(_clients[_iter], SERVERNAME, "461", "TOPIC :Not enough parameters"), void());
	if (!check_ChaOpCo_input(token))
		return;
	...
	if (token.size() == 2)
	{
		...
		sendRplErr(_clients[_iter].get_fd(), _clients[_iter].get_user_whole_str(), "TOPIC", token[1] + " :" + topic);
		return;
	}
	if (chan.get_topic_op())
		if (!chan.get_cha_cl_list().find(_clients[_iter].get_nick())->second.ch_operator)
			return (sendRplErr(_clients[_iter], SERVERNAME, "482", chan.get_channel_name() + " :You're not channel operator"), void());
